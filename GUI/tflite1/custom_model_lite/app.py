...
# Define paths to your models and label file
MODEL_PATHS = {
    "single_class_empty": "C:/Users/SAAD COMPUTER/OneDrive/Documents/tflite1/custom_model_lite/single.tflite",
}
LABEL_PATHS = {
    "single_class_empty": "C:/Users/SAAD COMPUTER/OneDrive/Documents/tflite1/custom_model_lite/labelmap.txt",
}

# Function to load the TFLite model and labels
def load_model(model_choice):
    interpreter = Interpreter(model_path=MODEL_PATHS[model_choice])
    ...
    with open(LABEL_PATHS[model_choice], 'r') as f:
...
def detect_objects(image, interpreter, input_details, output_details, labels):
    ...
    # Print output details to debug indices
    print(f"Output details: {output_details}")
    ...
    try:
        ...
        print(f"Error extracting output tensors: {e}")
        return image
    ...
    scores_list = [float(score[0]) for score in scores]
    ...
    for i in filtered_indices:
        ...
        class_name = "empty"  # Since this is a single-class model
        color = colors.get(class_name, (255, 255, 255))  # Default to white if class not found
        cv2.rectangle(image, (xmin, ymin), (xmax, ymax), color, 2)
        label = '{}: {:.2f}%'.format(class_name, scores[i][0] * 100)
...
def process_video(file_path, output_path, model_choice, frame_skip=5):
...
model_choice = 'single_class_empty'
interpreter, input_details, output_details, height, width, labels = load_model(model_choice)
...
def upload_and_detect():
    if request.method == 'POST':
        ...
        if media_type == 'image':
            ...
            process_video(video_path, output_path, model_choice)
